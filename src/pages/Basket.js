...
import { PizzaBasket } from '../components';
import { deleteAll as deleteAllAction } from '../redux/actions/basket';

const Basket = () => {

    const dispatch = useDispatch();

    const { totalPrice, totalCount, pizzas } = useSelector(({ basket }) => {
        return {
    ...
    const pizzaBlocks = Object.keys(pizzas).map(item => <PizzaBasket key={item} {...pizzas[item]}></PizzaBasket>)
    ...
    return (
        ...
        <div className="container container--cart">
            {Object.keys(pizzas).length ? <div className="cart">
 ...
 Корзина</h2>
                ...
                <div className="content__items--basket">
                    {
                        pizzaBlocks
                    }
                </div>
